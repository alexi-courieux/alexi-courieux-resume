...
function hexToRgb(hex: string) {
  const m = new RegExp(/^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i).exec(hex);
...
const DotGrid: React.FC<DotGridProps> = ({
  ...
  const circlePath = useMemo(() => {
    if (!globalThis.window?.Path2D) return null;
  ...
  useEffect(() => {
    ...
    if ('ResizeObserver' in globalThis.window) {
      ...
      (globalThis.window as Window).addEventListener('resize', buildGrid);
    }
    return () => {
      if (ro) ro.disconnect();
